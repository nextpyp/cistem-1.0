...
//	wxDateTime my_time_out;
	...
	FrealignParameterFileStats my_output_par_file(ouput_parameter_file, OPEN_TO_WRITE);
	FrealignParameterFileStats my_output_par_shifts_file(ouput_shift_file, OPEN_TO_WRITE, 16);
...
// Read whole parameter file to work out average values and variances
	...
	for (i = 0; i < refine_particle.number_of_parameters; i++)
	{
		parameter_average[i] /= input_par_file.all_lines;
		parameter_variance[i] /= input_par_file.all_lines;
...
//		search_projection_image.RotateFourier2DGenerateIndex(kernel_index, psi_max, psi_step, psi_start);
	...
	wxPrintf("\nAverage sigma noise = %f, average LogP = %f\nAverage ShiftX = %f, average ShiftY = %f\nSigma ShiftX = %f, sigma ShiftY = %f\nNumber of particles to refine = %i\n\n",
			parameter_average[14], parameter_average[13], parameter_average[4], parameter_average[5], sqrtf(parameter_variance[4]), sqrtf(parameter_variance[5]), images_to_process);
